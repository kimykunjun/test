...
namespace ACMS
{
	...
	/// </summary>
	public class FormNewSpaBooking1 : System.Windows.Forms.Form
	{
		...
		private DevExpress.XtraScheduler.Appointment myAppoinment;
		...
		public FormNewSpaBooking1(ACMSLogic.SpaBooking spaBooking, bool isWaitingList,
			DevExpress.XtraScheduler.Appointment appoinment)
		{
			//
			// Required for Windows Form Designer support
			//
		...
		public DateTime DtStartTime
		{
			set 
			{ 
				myDataTable.Rows[0]["dtStartTime"] = value;
				myDataTable.Rows[0]["dtEndTime"] = value.AddMinutes(15);
			}

			get { return ACMS.Convert.ToDateTime(myDataTable.Rows[0]["dtStartTime"]); }
		}
		...
		private void InitWaitingList()
		{
			if (myIsWaitingList)
			{
				this.Text = "New Waiting List";

				if (ACMS.Convert.ToDateTime(myDataTable.Rows[0]["dtStartTime"]).Hour < 20)
				{
					DateTime dtstart = new DateTime(dtEditDate.DateTime.Year, 
						dtEditDate.DateTime.Month, dtEditDate.DateTime.Day, 
						20, 0, 0);
				
					dtEdtStartTime.DateTime = dtstart;
				}

				dtEdtEndTime.Enabled = false;
			}
		}
		...
		{
			myDataTable = mySpaBooking.NewNormalSpaBooking();
			if (myIsWaitingList)
			{

				myDataTable.Rows[0]["nStatusID"] = 3; //waiting List
				
			}
			else
				myDataTable.Rows[0]["nStatusID"] = 2; //Booked
			...
			if (myAppoinment != null && !myIsWaitingList)
			{
				myDataTable.Rows[0]["dtDate"] = myAppoinment.Start.Date;
				myDataTable.Rows[0]["dtStartTime"] = myAppoinment.Start;
				myDataTable.Rows[0]["dtEndTime"] = myAppoinment.End;
				myDataTable.Rows[0]["nServiceEmployeeID"] = myAppoinment.ResourceId;
				
				dtEditDate.Enabled = false;
				dtEdtEndTime.Enabled = false;
				dtEdtStartTime.Enabled = false;
				lkpEdtTherapist.Enabled = false;
			}
		}
		/// <summary>
		...
		private void UcMemberEditValueChanged(string strMembershipID)
		{
			if (myMemberPackageLookupBuilder != null)
				myMemberPackageLookupBuilder.Refresh(strMembershipID);

			if (lkpEdtPackageID.Text.Length == 0)
			{
				myServiceCodeLookupBuilder.Refresh(-1);
			}
		}
		...
		private void SimpleButton56_Click_1(object sender, System.EventArgs e)
		{
			...
			if (myIsWaitingList)
			{
				if (dtEdtStartTime.DateTime.Hour < 16)
				{
					MessageBox.Show(this, "Waiting List must be set after 8.00 pm.");
					this.DialogResult = DialogResult.None;
					return;
				}
				else if (dtEdtStartTime.DateTime.Hour > 23 && dtEdtEndTime.DateTime.Hour > 23)
				{
					MessageBox.Show(this, "Waiting List must be set before 11.00 pm.");
					this.DialogResult = DialogResult.None;
					return;
				}
			}
			...
			try
			{
				mySpaBooking.Save(myDataTable, !myIsWaitingList);
				this.Close();
			}
			catch(Exception ex)
			{
		...
		private void dtEdtStartTime_EditValueChanged(object sender, System.EventArgs e)
		{
			DateTime dtstart = new DateTime(dtEditDate.DateTime.Year, dtEditDate.DateTime.Month, dtEditDate.DateTime.Day, 
				dtEdtStartTime.DateTime.Hour, dtEdtStartTime.DateTime.Minute, dtEdtStartTime.DateTime.Second);

			dtEdtEndTime.DateTime = dtstart.AddMinutes(15);
		}

		private void FormNewSpaBooking_Load(object sender, System.EventArgs e)
		{
			InitWaitingList();
		}
