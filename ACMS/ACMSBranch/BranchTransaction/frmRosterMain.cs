...
namespace ACMS.ACMSBranch.BranchTransaction
{
	...
	public class frmRosterMain : System.Windows.Forms.Form
	{
		...
		private void initRoster(string target,string _value)
		{
			...
			int selectedYear = System.Convert.ToInt32(cbYear.EditValue.ToString());
      

            if (WeekNo == 53 && System.Convert.ToInt32(selectedYear) == DateTime.Now.Year)
            {
                WeekNo = 53;
            }

            if (WeekNo == 53 && System.Convert.ToInt32(selectedYear) == DateTime.Now.Year + 1)
            {
                WeekNo = 1;
            ...
            myRoster = new Roster();
            DataTable tblRosterDetail = myRoster.GetRosterDetail(WeekNo, target, _value, RosterBranch, selectedYear);
		...
		{
			string strSQL;
			strSQL = "select strBranchCode,REPLACE(strBranchName,CHAR(10),'') AS strBranchName from tblBranch";
		...
		private void btnPrint_Click(object sender, System.EventArgs e)
		{
			...
			*/
			...
			_ds = new DataSet();
			SqlHelper.FillDataset(connection,CommandType.StoredProcedure,"up_get_RosterDetail",_ds,new string[] {"table"}, new SqlParameter("@weekNO", WeekNo),new SqlParameter("@target", target),new SqlParameter("@value", comboBoxRoster.EditValue.ToString()),new SqlParameter("@branchvalue", RosterBranch),new SqlParameter("@nYear", cbYear.EditValue) );
