...
namespace ACMS.ACMSManager.MasterData
{
	...
	public class frmPromotion : System.Windows.Forms.Form
	{
        ...
        internal DevExpress.XtraGrid.Views.Grid.GridView gvFreebies2;
...
//				UpdatePromotionSubItem(row);
        ...
        private void LoadFreebies()
		{
			...
			strSQL = "select strPromotionCode, B.strDescription, strItemCode, nMaxQty, IDNo from tblPromotionFreebie A join tblProduct B on A.strItemCode = B.strProductCode where strPromotionCode='" + dr["strPromotionCode"].ToString() + "'";
			if (this.txtItemCode.EditValue.ToString() != "")
		...
		private void btnFreebies_Add_Click(object sender, System.EventArgs e)
		{
            ...
            if (!dtFreebies.Columns.Contains("nMaxQty"))
            {
                DataColumn colMaxQty = new DataColumn("nMaxQty", typeof(int));
                dtFreebies.Columns.Add(colMaxQty);
            }
            foreach (int index in rowsHandle)
            {
            ...
            {
                DataRow rCopy = gvFreebies.GetDataRow(index);
                ((DataView)gvFreebies.DataSource).Table.Rows.Remove(rCopy);
            }		
			
			try
			{
				string strSQL = "select strPromotionCode, strItemCode, IDNo, nMaxQty from tblPromotionfreebie Where strPromotionCode = '" + dr["strPromotionCode"].ToString() + "'";
		...
		private void btnFreebies_DelAll_Click(object sender, System.EventArgs e)
		{
            ...
            for (int i = dtFreebies.Rows.Count - 1; i >= 0; i--)
            {
                ...
                rNew["strDescription"] = rCopy["strDescription"];
                rNew.EndEdit();
                //if (dtTemp.Select("strProductCode='" + rNew["strProductCode"] + "'").Length <=0)
                dtTemp.Rows.Add(rNew);
            }
            ...
            for (int i = dtFreebies.Rows.Count - 1; i >= 0; i--)
            {
                if (dtFreebies.Rows[i].RowState != DataRowState.Deleted)
                {
                    ...
                    dtFreebies.AcceptChanges();
                }
            }
            strSQL = "Delete From tblPromotionfreebie Where strPromotionCode = '" + dr["strPromotionCode"].ToString() + "' ";
            _ds = new DataSet();
            SqlHelper.FillDataset(connection, CommandType.StoredProcedure, "UP_GETDATA", _ds, new string[] { "table" }, new SqlParameter("@strSQL", strSQL));
            _ds.Dispose();
		...
		private void btnFreebies_Del_Click(object sender, System.EventArgs e)
		{
			...
			foreach(int index in rowsHandle)
			{
                ...
                strSQL = "Delete From tblPromotionfreebie Where strPromotionCode = '" + dr["strPromotionCode"].ToString() + "' And strItemCode='" + rCopy["strItemCode"].ToString() + "' ";
                _ds = new DataSet();
                SqlHelper.FillDataset(connection, CommandType.StoredProcedure, "UP_GETDATA", _ds, new string[] { "table" }, new SqlParameter("@strSQL", strSQL));
                _ds.Dispose();
			}
            ...
            for (int i = dtFreebies.Rows.Count - 1; i >= 0; i--)
            {
                foreach (int index2 in rowsHandle)
                {
                    if (i == index2)
                    {
                        gvFreebies2.DeleteRow(i);
                         dtFreebies.AcceptChanges();
                    }
                }                
            }            
		...
		private void LoadPackage()
		{
            ...
            string strSQL = "select strPackageCode,strDescription from (select strPackageCode,strDescription from tblPackage Where strPackageCode Not In (Select strPackageCode From tblPromotionPackage Where strPromotionCode = '" + dr["strPromotionCode"].ToString() + "') union select strCreditPackageCode as strPackageCode,strDescription from tblCreditPackage Where strCreditPackageCode Not In (Select strPackageCode From tblPromotionPackage Where strPromotionCode = '" + dr["strPromotionCode"].ToString() + "')) t";
			if (this.txtPcItemCode.EditValue.ToString() != "")
				Condition = " strPackageCode like '%" + this.txtPcItemCode.EditValue + "%' And";
		...
		private void btnPackage_Add_Click(object sender, System.EventArgs e)
		{
            ...
            if (!dtPackage.Columns.Contains("nMaxQty"))
            {
                DataColumn colMaxQty = new DataColumn("nMaxQty", typeof(int));
                dtPackage.Columns.Add(colMaxQty);
            }
            foreach (int index in rowsHandle)
            {
            ...
            {
                DataRow rCopy = gvPackage.GetDataRow(index);
                ((DataView)gvPackage.DataSource).Table.Rows.Remove(rCopy);
            }
		...
		{          
            ...
            strSQL = "Delete From tblPromotionPackage Where strPromotionCode = '" + dr["strPromotionCode"].ToString() + "' ";
            _ds = new DataSet();
            SqlHelper.FillDataset(connection, CommandType.StoredProcedure, "UP_GETDATA", _ds, new string[] { "table" }, new SqlParameter("@strSQL", strSQL));
            _ds.Dispose();
		...
		{		
            ...
            foreach (int index in rowsHandle)
            {
                ...
                strSQL = "Delete From tblPromotionPackage Where strPromotionCode = '" + dr["strPromotionCode"].ToString() + "' And strPackageCode='" + rCopy["strPackageCode"].ToString() + "' ";
                _ds = new DataSet();
                SqlHelper.FillDataset(connection, CommandType.StoredProcedure, "UP_GETDATA", _ds, new string[] { "table" }, new SqlParameter("@strSQL", strSQL));
                _ds.Dispose();
            }
        ...
        private void gvFreebies2_LostFocus(object sender, EventArgs e)
        {
            DataRow row = this.gvFreebies2.GetDataRow(gvFreebies2.FocusedRowHandle);
            if (row != null)
            {
                if (row["nMaxQty"] != DBNull.Value)
                {
                    gvFreebies2.CloseEditor();
                    gvFreebies2.UpdateCurrentRow();
                    //CreateCmdsAndUpdate(strSQL, (gvFreebies.DataSource as DataView).Table);

                    string strSQL = "update tblPromotionfreebie SET nMaxQty=" + row["nMaxQty"].ToString() + " where strPromotionCode='" + row["strPromotionCode"].ToString() + "' and strItemCode='" + row["strItemCode"] + "' ";
                    DataSet _ds = new DataSet();
                    SqlHelper.FillDataset(connection, CommandType.StoredProcedure, "UP_GETDATA", _ds, new string[] { "table" }, new SqlParameter("@strSQL", strSQL));
                    _ds.Dispose();
                }
        ...
        private void gvPackage2_LostFocus(object sender, EventArgs e)
        {
            DataRow row = this.gvPackage2.GetDataRow(gvPackage2.FocusedRowHandle);
            if (row != null)
            {
                if (row["nMaxQty"] != DBNull.Value)
                {
                    gvPackage2.CloseEditor();
                    gvPackage2.UpdateCurrentRow();
                    //CreateCmdsAndUpdate(strSQL, (gvFreebies.DataSource as DataView).Table);

                    string strSQL = "update tblPromotionPackage SET nMaxQty=" + row["nMaxQty"].ToString() + " where strPromotionCode='" + row["strPromotionCode"].ToString() + "' and strPackageCode='" + row["strPackageCode"] + "' ";
                    DataSet _ds = new DataSet();
                    SqlHelper.FillDataset(connection, CommandType.StoredProcedure, "UP_GETDATA", _ds, new string[] { "table" }, new SqlParameter("@strSQL", strSQL));
                    _ds.Dispose();
                }
