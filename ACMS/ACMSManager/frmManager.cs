...
namespace ACMS
{
	public class frmManager : System.Windows.Forms.Form
	{
        ...
        private static ACMS.ACMSManager.MasterData.frmRoadShow frmRoadShow;
        ...
        private static ACMS.ACMSManager.MasterData.frmCashVoucher frmCashVoucher;
        ...
        private static ACMS.ACMSManager.MasterData.frmSalesTarget frmSalesTarget;
        private static ACMS.ACMSManager.MasterData.frmReceipt frmReceipt;
        ...
        private DevExpress.XtraEditors.SimpleButton lblOne_4;
        ...
        private DevExpress.XtraEditors.SimpleButton lblThree_4;
        ...
        private SimpleButton lblOne_3;
        private GroupControl groupCashCardMaster;
        internal GroupControl groupCashCard;
        private Label label19;
        internal SimpleButton btnCashCardImport;
        private PictureBox pbSelect;
        private TextEdit txtFilePath;
        private GroupControl groupCashVoucherMaster;
        internal GroupControl groupCashVoucher;
        private Label label20;
        internal SimpleButton btnCashVoucherImport;
        private PictureBox pbSelectCashVoucher;
        private TextEdit txtFilePathCashVoucher;
        private Label label9;
        private LookUpEdit comboBoxHall; 
		User oUser;
        ...
        private static ACMS.ACMSManager.Human_Resource.Reports.RPCashCardReport rptCashCard;                
        ...
        internal DevExpress.XtraEditors.SimpleButton btnUpdateClassSchedule;
        ...
        internal DevExpress.XtraEditors.ComboBoxEdit cmbxScheduleType;
        ...
        private DevExpress.XtraEditors.GroupControl grpNextSemSchedule;
		...
		private void ClassScheduleInit()
		{
            ...
            strSQL = " select strBranchName, strBranchCode from tblBranch where nBrStatusID=1 and strBranchCode not in ('LC','HQ','BV') ";
        ...
        private void _Click(object sender, System.EventArgs e)
        {
            ...
            string strSQL = "SELECT nClassScheduleID, strBranchCode, nDay, dtStartTime, dtEndTime, " +
                                    "strClassCode, nHallNo, nInstructorID, fFree, COALESCE(fPeak, -1) AS fPeak, " +
                                    "fAllowStudentOnPeak, fReservation, nMaxNo, fAllowUOBBooking, nCommissionTypeID,dtStartDate,dtEndDate " +
                                "FROM tblClassSchedule " +
                                "WHERE nClassScheduleID = " + cc.ClassScheduleID;
            DataSet _ds = new DataSet();
            SqlHelper.FillDataset(connection, CommandType.StoredProcedure, "UP_GETDATA", _ds, new string[] { "table" }, new SqlParameter("@strSQL", strSQL));
        ...
        private void btnUpdateClassSchedule_Click(object sender, System.EventArgs e)
        {
            try
            {
                if (!UI.ShowYesNoMessage(this, "Are you sure you want to update outlets schedule from master schedule?\nThis will take about 2 minutes to complete."))
                    return;
                SqlHelper.ExecuteNonQuery(connection, CommandType.StoredProcedure, "SP_AutoCreateClassInstance");
                MessageBox.Show("Outlets' schedule updated!");
                //Raymond FTP Class Schedule
                HttpWebRequest myRequest = (HttpWebRequest)WebRequest.Create("http://web.amorefitness.com/AmoreClassSchedule/AmoreClassSchedule.aspx?ftp=2");
		...
		private void scheduleRefresh()
		{
            ...
            if (cmbxScheduleType.Text == "Current Schedule")
                strSQL = "select nClassScheduleID,strBranchCode,nDay,dtStartTime,dtEndTime,strClassCode,nHallNo,nInstructorID,fFree,fPeak,fAllowStudentOnPeak,fReservation," +
                "nMaxNo,fAllowUOBBooking,nCommissionTypeID,dtStartDate,dtEndDate,Convert(varchar(9),dtStartDate,6)+' - '+ISNULL(Convert(varchar(9),dtEndDate,6),'~') as strPeriod from tblClassSchedule " +
                "where nHallNo = '" + comboBoxHall.EditValue.ToString() + "' and strBranchCode='" + comboBoxBranch.EditValue.ToString() + "' and getdate()>dtStartDate and (dtEndDate is null or dtEndDate>=Convert(varchar(10),GETDATE(),102))";
            else
                strSQL = "select nClassScheduleID,strBranchCode,nDay,dtStartTime,dtEndTime,strClassCode,nHallNo,nInstructorID,fFree,fPeak,fAllowStudentOnPeak,fReservation," +
                "nMaxNo,fAllowUOBBooking,nCommissionTypeID,dtStartDate,dtEndDate,Convert(varchar(9),dtStartDate,6)+' - '+ISNULL(Convert(varchar(9),dtEndDate,6),'~') as strPeriod from tblClassSchedule " +
                "where nHallNo = '" + comboBoxHall.EditValue.ToString() + "' and strBranchCode='" + comboBoxBranch.EditValue.ToString() + "' and dtStartDate>getdate()";
			...
			foreach(DataRow dr in _dtClassSchedule.Rows)
			{
				if (dr["nDay"].ToString()=="1")
				{
                    ...
                    classComponent1.lblClassTime.Text = Convert.ToDateTime(dr["dtStartTime"]).ToString("hh:mm tt") + "-" + Convert.ToDateTime(dr["dtEndTime"]).ToString("hh:mm tt");
					...
					classComponent1.lblPeriod.Text = dr["strPeriod"].ToString();
                    classComponent1.lblClassScheduleID.Text = dr["nClassScheduleID"].ToString();
					nMon = nMon + 50;
	...
	*/
		...
		private void mdInit()
		{
            ...
            frmRoadShow = new ACMS.ACMSManager.MasterData.frmRoadShow();
            ...
            frmCashVoucher = new ACMS.ACMSManager.MasterData.frmCashVoucher();
            ...
            frmSalesTarget = new ACMS.ACMSManager.MasterData.frmSalesTarget();
            frmReceipt = new ACMS.ACMSManager.MasterData.frmReceipt();
        ...
        private void mnuMdCashVoucher()
        {
            if (!frmCashVoucher.IsDisposed)
            {
                frmCashVoucher.Dispose();
            }

            frmCashVoucher = new ACMS.ACMSManager.MasterData.frmCashVoucher();
            panel1.Controls.Clear();
            frmCashVoucher.TopLevel = false;
            frmCashVoucher.Dock = DockStyle.Fill;
            frmCashVoucher.Parent = panel1;
            frmCashVoucher.Show();
            panel1.Refresh();
        ...
        private void mnuMdRoadShow()
        {
            if (!frmRoadShow.IsDisposed)
            {
                frmRoadShow.Dispose();
            }
            ...
            frmRoadShow = new ACMS.ACMSManager.MasterData.frmRoadShow();
            panel1.Controls.Clear();
            frmRoadShow.TopLevel = false;
            frmRoadShow.Dock = DockStyle.Fill;
            frmRoadShow.Parent = panel1;
            frmRoadShow.Show();
            panel1.Refresh();
        }
        ...
        private void mnuMdSalesTarget()
		{
			if(!frmSalesTarget.IsDisposed)
			{
				frmSalesTarget.Dispose();
			}
			
			frmSalesTarget = new ACMS.ACMSManager.MasterData.frmSalesTarget();
			panel1.Controls.Clear();
			frmSalesTarget.TopLevel = false;
            frmSalesTarget.Dock = DockStyle.Fill;
            frmSalesTarget.Parent = panel1;
            frmSalesTarget.Show();
			panel1.Refresh();

		}

        private void mnuMdReceipt()
        {
            if (!frmReceipt.IsDisposed)
            {
                frmReceipt.Dispose();
            }
            ...
            frmReceipt = new ACMS.ACMSManager.MasterData.frmReceipt();
            panel1.Controls.Clear();
            frmReceipt.TopLevel = false;
            frmReceipt.Dock = DockStyle.Fill;
            frmReceipt.Parent = panel1;
            frmReceipt.Show();
            panel1.Refresh();
...
//					}
		...
		{
            if (employee.CanAccess("AM_VIEW_MONTH_TO_MONTH_SALES_REPORT", terminalUser.Branch.Id))
			{
				try
				{
			        ReportPanel.Controls.Clear();
		...
		{
            if (employee.CanAccess("AM_VIEW_SALES_MANAGEMENT", terminalUser.Branch.Id))
			{
				try
				{
			        ReportPanel.Controls.Clear();
		...
		{
            if (employee.CanAccess("AM_VIEW_SPA_DAILY_BRANCH_SALES", terminalUser.Branch.Id))
			{
				try
				{
			        ACMSLogic.User oUser = new User();
		...
		{
            if (employee.CanAccess("AM_VIEW_FITNESS_DAILY_BRANCH_SALES", terminalUser.Branch.Id))
			{
				try
				{
			        ACMSLogic.User oUser = new User();
        ...
        private void show_Accounts2()
        {
            ACMSLogic.User oUser = new User();

            ReportPanel.Controls.Clear();
            rptCashCard = new ACMS.ACMSManager.Human_Resource.Reports.RPCashCardReport(employee.Id);
            rptCashCard.TopLevel = false;
            rptCashCard.Dock = DockStyle.Fill;
            rptCashCard.Parent = ReportPanel;
            rptCashCard.Show();
            ReportPanel.Refresh();
        }
	...
	private void bbiManagerTimeCard_ItemClick(object sender, DevExpress.XtraBars.ItemClickEventArgs e)
		...
		private void int_lblOne()
		{
            ...
            groupCashCardMaster.Hide();
            groupCashVoucherMaster.Hide();
			lblOne_1.ForeColor=System.Drawing.Color.MediumBlue;
			lblOne_2.ForeColor=System.Drawing.Color.MediumBlue;
            lblOne_3.ForeColor = System.Drawing.Color.MediumBlue;
            lblOne_4.ForeColor = System.Drawing.Color.MediumBlue;

            panel3.Controls.Clear();
            frmCashVoucher.TopLevel = false;
            frmCashVoucher.Dock = DockStyle.Fill;
            frmCashVoucher.Parent = panel3;
            frmCashVoucher.Show();
            panel3.Refresh();
		}
		...
		private void int_lblThree()
		{
            ...
            lblThree_4.ForeColor = System.Drawing.Color.MediumBlue;
		}
		...
		private void lblThree_3_Click(object sender, System.EventArgs e)
		{
            ...
            //myMemberCardPhoto.Parent = grpRoadShow;
            //grpRoadShow.Refresh();
        ...
        private void lblThree_4_Click(object sender, System.EventArgs e)
        {
            int_lblThree();
            lblThree_4.ForeColor = System.Drawing.Color.Firebrick;
            this.grpNextSemSchedule.Show();

            grpNextSemSchedule.SuspendLayout();
            ...
            //ACMSLogic.User oUser = new User();
            //myMemberCardPhoto = new ACMS.ACMSManager.Operation.frmMemberCardPhoto();
            //myMemberCardPhoto.initData(oUser);
            //myMemberCardPhoto.SetEmployeeRecord(employee);
            //myMemberCardPhoto.SetTerminalUser(terminalUser);
            ...
            //grpRoadShow.Refresh();
            //myMemberCardPhoto.Show();
            grpNextSemSchedule.ResumeLayout();
...
//			ReportPanel.Refresh();
        ...
        private void lblOne_3_Click(object sender, EventArgs e)
        {
            int_lblOne();
            lblOne_3.ForeColor = System.Drawing.Color.Firebrick;
            groupCashCardMaster.Show();
        }

        private void lblOne_4_Click(object sender, EventArgs e)
        {
            int_lblOne();
            lblOne_4.ForeColor = System.Drawing.Color.Firebrick;
            groupCashVoucherMaster.Show();
        }
        ...
        private void pbSelectCashVoucher_Click(object sender, EventArgs e)
        {
            OpenFileDialog fDialog = new OpenFileDialog();
            fDialog.Title = "Open File to import";
            fDialog.Filter = "Scard Files|*.*";
            ...
            if (fDialog.ShowDialog() == DialogResult.OK)
            {
                txtFilePathCashVoucher.Text = fDialog.FileName.ToString();
            }
        }
        ...
        private void btnCashVoucherImport_Click(object sender, EventArgs e)
        {
            try
            {
                string strSN = "", strDescription = "", strDescription2 = "", dtStartDate = "", dtExpiryDate = "";
                string mValue = "", strBranchCode = "", nCashVoucherType = "", nValidDuration = "", strDurationUnit = "", fSell = "";

                if (VerifyCashVoucherFile())
                {
        ...
        private void btnCashCardImport_Click(object sender, EventArgs e)
        {
            try
            {                
                string strLine = "", strBal = "", strSN = "", strCAN = "", strRecNo = "";
                string strTransType = "", strTimeStamp = "", strPurAmt = "", strMerchant = "";
                if (VerifyFile())
                {
                    StreamReader objstream = new StreamReader(txtFilePath.Text);
                    int i = 1;
                    while (!objstream.EndOfStream)
                    {
                        ...
                        if (i == 3)
                        {
                            ...
                            double Num;
                            if (double.TryParse(arrLine[1].ToString().Replace("S$", "").Trim(), out Num))
                                strBal = arrLine[1].ToString().Replace("S$", "").Trim();
                        }
                        if (i == 4)
                        {
                            string[] arrLine;
                            ...
                            if (arrLine.GetUpperBound(0) > 0)
                                strSN = arrLine[1].ToString().Trim();
                        }
                        if (i == 5)
                        {
                            string[] arrLine;
                            ...
                            if (arrLine.GetUpperBound(0) > 0)
                                strCAN = arrLine[1].ToString().Trim();
                        }
                        if (i % 6 == 2 && i != 2)
                        {
                            if (strLine.Trim() != "")
                            {
                                ...
                                arrLine = strLine.Split(':');
                                int Num;
                                if (int.TryParse(arrLine[1].ToString().Trim(), out Num))
                                    strRecNo = arrLine[1].ToString().Trim();                                   
                            }                            
                        }
                        if (i % 6 == 3 && i != 3)
                        {
                            if (strLine.Trim() != "")
                            {
                                ...
                                if (arrLine.GetUpperBound(0) > 0)
                                    strTransType = arrLine[1].ToString().Trim();
                            }                            
                        }
                        if (i % 6 == 4 && i != 4)
                        {
                            ...
                            if (arrLine.GetUpperBound(0) > 0)
                                strTimeStamp = string.Format("{0:yyyy-MM-dd HH:mm:ss}", Convert.ToDateTime(arrLine[1].ToString().Trim()));
                        }
                        if (i % 6 == 5 && i != 5)
                        {
                            string[] arrLine;
                            arrLine = strLine.Split('=');
                            double Num;
                            if (double.TryParse(arrLine[1].ToString().Trim(), out Num))
                                strPurAmt = arrLine[1].ToString().Trim();
                        }
                        if (i % 6 == 0 && i != 6)
                        {
                            string[] arrLine;
                            ...
                            if (arrLine.GetUpperBound(0) > 0)
                                strMerchant = arrLine[1].ToString().Trim();

                            int output = 0;
                            if (strLine != "" && strBal != "" && strSN != "" && strCAN != "" && strRecNo != "" &&
                                strTransType != "" && strTimeStamp != "" && strPurAmt != "" && strMerchant != "")
                            {
                                SqlHelper.ExecuteNonQuery(connection, "In_tblCashCard",
                                    new SqlParameter("@retval", output),
                                    new SqlParameter("@strImportBy", User.EmployeeID),
                                    new SqlParameter("@Bal", Convert.ToDouble(strBal)),
                                    new SqlParameter("@strSN", strSN),
                                    new SqlParameter("@strCAN", strCAN),
                                    new SqlParameter("@RecNo", Convert.ToInt16(strRecNo)),
                                    new SqlParameter("@strTransType", strTransType),
                                    new SqlParameter("@dtTimeStamp", strTimeStamp),
                                    new SqlParameter("@PurAmt", Convert.ToDouble(strPurAmt)),
                                    new SqlParameter("@strMerchant", strMerchant));
                            }
                        }
                        i++;                        
                    }
        ...
        private bool VerifyCashVoucherFile()
        {
            if (txtFilePath.Text != "")
            {
                ...
                using (OleDbConnection dbConn = new OleDbConnection(connString))
                {
                    using (OleDbCommand cmd = new OleDbCommand(@"SELECT * FROM [Sheet1$]", dbConn))
                    {
                        ...
                        using (OleDbDataReader reader = cmd.ExecuteReader())
                        {
                            ...
                            while (reader.Read())
                            {
                                if (reader[0].ToString() != "" && reader[1].ToString() != "" && reader[5].ToString() != "" && reader[6].ToString() != "" && reader[7].ToString() != "" && 
                                    reader[8].ToString() != "" && reader[9].ToString() != "" && reader[10].ToString() != "")
                                {
                                    ...
                                    row["nCashVoucherType"] = reader[7].ToString();
                                    row["nValidDuration"] = reader[8].ToString();
                                    row["strDurationUnit"] = reader[9].ToString();
                                    row["fSell"] = reader[10].ToString();
                                    table.Rows.Add(row);
                                    i++;
                                }
                                else
        ...
        private bool VerifyFile()
        {
            if (txtFilePath.Text != "")
            {
                ...
                while (!objstream.EndOfStream)
                {
                    ...
                    if (i == 1 && !strLine.Contains("Beginning of File"))
                    {
                        MessageBox.Show(this, "Wrong File format.\nCheck line #" + i.ToString());
                        return false;
                    }
                    if (i == 3)
                    {
                        if (!strLine.Contains("Amount Left on CashCard"))
                        {
                            MessageBox.Show(this, "Wrong File format.\nCheck line #" + i.ToString());
                            return false;
                        }
                    ...
                    if (i == 4)
                    {
                        if (!strLine.Contains("Card Serial Number"))
                        {
                            MessageBox.Show(this, "Wrong File format.\nCheck line #" + i.ToString());
                            return false;
                        }
                    ...
                    if (i % 6 == 2 && i != 2)
                    {
                        if (!strLine.Contains("Record Number :"))
                        {
                            if (strLine.Trim() == "")
                            {
                                strLine = objstream.ReadLine();
                    ...
                    if (i % 6 == 3 && i != 3)
                    {
                        if (!strLine.Contains("Transaction Type ="))
                        {
                            MessageBox.Show(this, "Wrong File format.\nCheck line #" + i.ToString());
                            return false;
                        }
                    ...
                    if (i % 6 == 5 && i != 5)
                    {
                        if (!strLine.Contains("Purchase Amount (S$) ="))
                        {
                            MessageBox.Show(this, "Wrong File format.\nCheck line #" + i.ToString());
                            return false;
                        }
                    ...
                    if (i % 6 == 1 && i != 7 && i != 1)
                    {
                        if (!strLine.Contains("End of Record"))
                        {
                            MessageBox.Show(this, "Wrong File format.\nCheck line #" + i.ToString());
                            return false;
                        }
