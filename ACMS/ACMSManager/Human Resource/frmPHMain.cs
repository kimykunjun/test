...
{
	/// <summary>
	/// Summary description for frmPHMain.
	/// </summary>
	public class frmPHMain : System.Windows.Forms.Form
	{
		...
		private void LoadOvertime()
		{
			...
			sql = "";
            strSQL = "dw_tblLeavePH_SelectAll";
            ...
            sql = "dtDate >= '" + thisMonth.ToLongDateString() + "' And dtDate < '" + NextMonth.ToShortDateString() + "'";
		...
		{	
			...
			if (gridViewOverTime.RowCount > 0)
			{
				...
				{                  
                    try
                    {
                        SqlHelper.ExecuteNonQuery(connection, "dw_tblLeavePH_UpdateStatus",
                            new SqlParameter("@Employee", Convert.ToInt32(dr["nEmployeeID"])),
                            new SqlParameter("@dtDate", Convert.ToDateTime(dr["dtDate"])),
                            new SqlParameter("@ManagerID", Convert.ToInt32(oUser.NEmployeeID())));
                    }
		...
		private void btnReject_Click(object sender, System.EventArgs e)
		{
			if (gridViewOverTime.RowCount > 0)
			{
                ...
                if (dr != null)
                {
                    try
                    {
                        SqlHelper.ExecuteNonQuery(connection, "dw_tblLeavePH_UpdateCancel",
                            new SqlParameter("@Employee", Convert.ToInt32(dr["nEmployeeID"].ToString())),
                            new SqlParameter("@dtDate", Convert.ToDateTime(dr["dtDate"])),
                            new SqlParameter("@ManagerID", oUser.NEmployeeID())
                            );
		...
		private void frmPHMain_Load(object sender, System.EventArgs e)
		{
