...
namespace ACMS.ACMSManager.Reports
{
	...
	/// </summary>
	public class RPActivePackage : DevExpress.XtraEditors.XtraForm
	{
		...
		{
			DataSet _ds = new DataSet();
            string strSQL = "SELECT X.strReportGroup," + BranchParam1 + "," + BranchParam3 + " FROM (SELECT " + BranchParam2;
            strSQL += " ,I.strReportGroup FROM ( ";
            strSQL += " SELECT sum(tblReceiptPayment.mAmount) as Amount,tblReceipt.strbranchcode,'Active Package' as strReportGroup FROM tblReceiptEntries INNER JOIN ";
            strSQL += " tblReceipt ON tblReceiptEntries.strReceiptNo = tblReceipt.strReceiptNo INNER JOIN";
            strSQL += " tblReceiptPayment ON tblReceipt.strReceiptNo = tblReceiptPayment.strReceiptNo INNER JOIN";
            strSQL += " tblPackage ON tblReceiptEntries.strCode = tblPackage.strPackageCode";
            strSQL += " WHERE     (tblPackage.fActivePkg = 1) and tblReceipt.fvoid = 0 and ";
            strSQL += " tblReceipt.dtDate between convert(datetime,'" + dtStartDate + "',103) and convert(datetime,'" + dtEndDate + "',103)";
            strSQL += " group by tblReceipt.strbranchcode ";
